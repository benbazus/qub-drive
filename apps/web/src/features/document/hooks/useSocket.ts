...
import { CollaborationRole, ConnectionStatus, DocumentChange, User } from '../components/types';
...
export const useSocket = ({
    ...
    const connect = useCallback(() => {
        ...
        // Periodic forced save fallback: ensure server persists latest content every 30s when connected
        let forcedSaveInterval: NodeJS.Timeout | null = null;
        const startForcedSave = () => {
            if (forcedSaveInterval) return;
            forcedSaveInterval = setInterval(() => {
                if (socketRef.current?.connected) {
                    // Emit save-document with empty title/content as a ping-save
        ...
        const stopForcedSave = () => {
            if (forcedSaveInterval) {
                clearInterval(forcedSaveInterval);
                forcedSaveInterval = null;
            }
        };
