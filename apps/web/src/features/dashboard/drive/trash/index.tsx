...
const TrashPage = () => {
    ...
    const trashedFiles = finalFilteredFiles.filter((file: { deleted: any; }) => file.deleted);

    console.log(" 7777777777777777777777777777777 ")
    console.log(finalFilteredFiles)
    console.log(" 7777777777777777777777777777777 ")
    ...
    return (
        <>
            ...
            <DashboardLayout>
                ...
                <Card className="overflow-hidden">
                    <CardHeader className="flex flex-row items-center justify-between">
                        <nav className='hidden sm:flex items-center space-x-1 sm:space-x-2 text-sm min-w-0 flex-1'>
                            {/* The breadcrumb logic might need adjustment if you want to navigate within trash,
                            but for a simple trash view, this is fine. */}
                    ...
                    {trashedFiles.length === 0 && !isLoadingFiles && ( // 👈 Use the new filtered array for the check
                    ...
                    <CardContent className='p-0'>
                        {layout === 'grid' ? (
                            <FileGrid
                                files={trashedFiles} // 👈 Use the new filtered array
                ...
                <DialogManager dialogs={dialogs} closeDialog={closeDialog} files={trashedFiles} /> {/* 👈 Pass trashed files to dialogs */}
            </DashboardLayout>
        </>
    );
};
