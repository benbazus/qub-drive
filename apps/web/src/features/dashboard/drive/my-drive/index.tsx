...
const MyDrivePage = () => {
    ...
    // 👇 Filter out files that are trashed, shared, or starred
    const myDriveFiles = finalFilteredFiles.filter((file: { deleted: any; shared: any; starred: unknown; }) =>
        !file.deleted && !file.shared && !file.starred
    );
        ...
        <DashboardLayout>
            ...
            <Card className="overflow-hidden">
                ...
                <CardContent className='p-0'>
                    {myDriveFiles.length === 0 && !isLoadingFiles ? ( // 👈 Use the new filtered array for the check
                        <div className="text-center p-8 text-gray-500">
                            <EmptyState />
                        </div>
                    ) : layout === 'grid' ? (
                        <FileGrid
                            files={myDriveFiles} // 👈 Use the new filtered array
            ...
            <DialogManager dialogs={dialogs} closeDialog={closeDialog} files={myDriveFiles} /> {/* 👈 Pass filtered files to dialogs */}
        </DashboardLayout>
    );
};
